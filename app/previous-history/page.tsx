...
type Submission = {
  ...
  tableName: string;
  bugNumber?: number;
  stage?: string | null; // Add stage to the Submission type
  details: any;
};
...
export default function HistoryPage() {
    ...
    const fetchSubmissions = async () => {
      ...
      const formattedSubmissions: Submission[] = [
        ...
        ...projectRes.data?.map((pr) => ({
          ...
          stage: pr.stage, // Include stage
          details: pr,
        })) || [],
      ];

      // Sort submissions by type (alphabetically) and then by submissionDate (most recent first)
  ...
  return (
    <div className="space-y-6 p-12">
      ...
      <Table>
        ...
        <TableHeader>
          <TableRow>
            ...
            <TableHead>Stage</TableHead> {/* Add Stage column */}
        ...
        <TableBody>
          {filteredSubmissions.map((submission) => (
            <TableRow key={submission.id}>
              ...
              <TableCell>{submission.stage || "N/A"}</TableCell> {/* Display stage or "N/A" if null */}
              <TableCell>
      ...
      <SubmissionDetailsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} submission={selectedSubmission} />
